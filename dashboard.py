...
# Mapping persis seperti notebook asli
...
# Kolom bantu di day_df untuk filtering (tidak mengubah data asli)
...
# Filter Tahun
year_options = sorted(hour_df['yr'].unique().tolist())
...
# Filter Cuaca
weather_options = hour_df['weathersit'].dropna().unique().tolist()
...
# =====================================================
# APPLY FILTERS — TIDAK MENGUBAH day_df / hour_df ASLI
# =====================================================
# Filter hour
...
filtered_day = filtered_day[
    (filtered_day['yr_label'].isin(year_filter)) &
    (filtered_day['season_label'].isin(season_filter)) &
    (filtered_day['weather_label'].isin(weather_filter))
]
...
# =====================================================
# KPI METRICS (selalu pakai data asli penuh + filtered)
# =====================================================
total_all    = day_df['cnt'].sum()
highest_all  = day_df['cnt'].max()
avg_day_all  = day_df['cnt'].mean()
avg_hour_all = hour_df['cnt'].mean()

total_filt   = filtered_day['cnt'].sum()   if len(filtered_day) > 0 else 0
highest_filt = filtered_day['cnt'].max()   if len(filtered_day) > 0 else 0
...
with tab1:
    ...
    if len(filtered_day) == 0:
        ...
        # Sub kolom: bar per tahun + pie pengguna
        ...
        with col_b:
            ...
            registered_t = filtered_day['registered'].sum()
            fig3, ax3 = plt.subplots(figsize=(5, 3.5))
            ax3.pie([casual_t, registered_t],
...
# ─────────────────────────────────────────────────────
# TAB 2 — MUSIM & CUACA  (Pertanyaan 1 asli tetap ada)
# ─────────────────────────────────────────────────────
with tab2:
    ...
    if len(filtered_day) == 0 or len(filtered_hour) == 0:
        st.warning("⚠️ Tidak ada data untuk filter yang dipilih.")
    else:
        # Toggle Bar / Line
        chart_type = st.radio(
        ...
        # ── Plot musim & cuaca (SAMA PERSIS dengan kode asli, pakai filtered_day) ──
        ...
        if chart_type == "Bar Chart":
            bars_s = axes[0].bar(season_avg.index, season_avg.values, color=colors_s)
...
# ─────────────────────────────────────────────────────
# TAB 3 — ANALISIS WAKTU  (Pertanyaan 2 asli tetap ada)
# ─────────────────────────────────────────────────────
with tab3:
    ...
    if len(filtered_hour) == 0:
        ...
        # ── Plot per jam & per hari (SAMA PERSIS dengan kode asli) ──
        ...
        # ── Bonus: Hari Kerja vs Libur per jam ──
...
# ─────────────────────────────────────────────────────
# TAB 4 — ANALISIS LANJUTAN  (persis sama dengan kode asli)
# ─────────────────────────────────────────────────────
with tab4:
    ...
    if len(filtered_day) == 0:
        ...
        # ── Ringkasan Statistik (SAMA dengan kode asli) ──
        ...
        # ── 4 subplot (SAMA PERSIS dengan kode asli) ──
        ...
        # ── Bonus: Tren Casual vs Registered per bulan ──
...
# ─────────────────────────────────────────────────────
# TAB 5 — KESIMPULAN  (SAMA PERSIS dengan kode asli)
# ─────────────────────────────────────────────────────
with tab5:
